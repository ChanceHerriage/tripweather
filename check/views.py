...
def check_weather(request, unique_id=None):
	...
	if request.method == "POST" or trip:
		if trip_form.is_valid() or trip:
			if trip:
				s_response = requests.get("http://api.openweathermap.org/data/2.5/weather?APPID=152e2a47ac3c0d92b92e386a722f8217&q=" + trip.start_loc)
				e_response = requests.get("http://api.openweathermap.org/data/2.5/weather?APPID=152e2a47ac3c0d92b92e386a722f8217&q=" + trip.end_loc)
			else:
				s_response = requests.get("http://api.openweathermap.org/data/2.5/weather?APPID=152e2a47ac3c0d92b92e386a722f8217&q=" + trip_form.cleaned_data['start_loc'])
				e_response = requests.get("http://api.openweathermap.org/data/2.5/weather?APPID=152e2a47ac3c0d92b92e386a722f8217&q=" + trip_form.cleaned_data['end_loc'])
			...
			if e_cur_temp < s_cur_temp:
				trip_notes = 'Oooohhhh. Looks like {0} is a little bit cooler than {1}. Safe travels!'.format(s_response['name'], e_response['name'])
